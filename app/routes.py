...
def login():
    if request.method == "POST":
        ...
        if CHALLENGE == Challenge.LOGIN_SQLI:
            ...
            user = db.sql_query(f"SELECT id, username FROM users WHERE username = ? AND password = ?", [username, password])
        else:
...
def changepwd():
    if request.method == "POST":
        ...
        if CHALLENGE == Challenge.LOGIN:
            ...
            db.sql_insert(f"UPDATE users SET password = '{new_pwd}' WHERE username = '{user['username']}'")
